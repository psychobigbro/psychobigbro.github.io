...
function onGetPastRaceDatesSuccess (result,status,xhr) {
	...
	if (Array.isArray(raceDates) && raceDates.length > 0) {
		...
		$("#query-form").find("p").text("Dates refreshed");
	}
	else
		$("#query-form").find("p").text("No dates refreshed");
	
	$("#query-btn").button('enable');
}

function onSaveInstanceIDSuccess (result,status,xhr) {
	console.log("***onSaveInstanceSuccess called!!!");
	$("#sys-msg").text(result.status + ": " + result.message + " " + result.response);
	if (status != "timeout" && result.status == "ACK")
		//then assume server got the token
...
function preSubmitBetQuery () {
	...
	$("#query-form").find("p").text("Working ...");
